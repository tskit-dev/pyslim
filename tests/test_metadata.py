...
class TestMetadataSchemas(tests.PyslimTestCase):
    ...
    def test_slim_metadata(self, basic_recipe):
        tables = basic_recipe["ts"].tables
    ...
    def test_slim_metadata_schema_equality(self, basic_recipe):
        t = basic_recipe["ts"].tables
...
class TestTreeSequenceMetadata(tests.PyslimTestCase):
    arbitrary_basic_recipe = [next(basic_recipe_eq())]  # for testing any one recipe
    ...
    @pytest.mark.parametrize('basic_recipe', arbitrary_basic_recipe, indirect=True)    
    def test_set_tree_sequence_metadata_errors(self, basic_recipe):
        tables = basic_recipe["ts"].tables
    ...
    @pytest.mark.parametrize('basic_recipe', arbitrary_basic_recipe, indirect=True)    
    def test_set_tree_sequence_metadata_keeps(self, basic_recipe):
    ...
    @pytest.mark.parametrize('basic_recipe', arbitrary_basic_recipe, indirect=True)    
    def test_set_tree_sequence_metadata(self, basic_recipe):
        tables = basic_recipe["ts"].tables
    ...
    def test_WF_model_type(self, basic_recipe):
        self.validate_model_type(basic_recipe["ts"], "WF")
        
    @pytest.mark.parametrize('basic_recipe', basic_recipe_eq("nonWF"), indirect=True)    
    def test_nonWF_model_type(self, basic_recipe):
        self.validate_model_type(basic_recipe["ts"], "nonWF")
        
    @pytest.mark.parametrize(
        'basic_recipe', basic_recipe_eq(exclude_if_has_key="user_metadata"), indirect=True)    
    def test_recover_metadata(self, basic_recipe):
    ...
    @pytest.mark.parametrize('basic_recipe', basic_recipe_eq("user_metadata"), indirect=True)    
    def test_user_metadata(self, basic_recipe):
        ts = basic_recipe["ts"]
        md = ts.metadata["SLiM"]
        #print(md)
...
class TestAlleles(tests.PyslimTestCase):
    ...
    def test_haplotypes(self, basic_recipe):
        slim_ts = basic_recipe["ts"]
...
class TestNucleotides(tests.PyslimTestCase):
    ...
    def test_nucleotides(self, basic_recipe):
        '''
...
class TestDecoding(tests.PyslimTestCase):
    ...
    def test_decoding(self, basic_recipe):
        tables = basic_recipe["ts"].tables
...
class TestMetadataAttributeError(tests.PyslimTestCase):
    """
    These are all only tested on a single recipe, the first grabbed from basic_recipe
    """
    
    def test_population_error(self, basic_recipe):
        ts = basic_recipe["ts"]
    ...
    def test_individual_error(self, basic_recipe):
        ts = basic_recipe["ts"]
    ...
    def test_node_error(self, basic_recipe):
        ts = basic_recipe["ts"]
        for x in ts.nodes():
            if x.metadata is not None:
                with pytest.raises(AttributeError) as exec_info:
    ...
    def test_mutation_error(self, basic_recipe):
        ts = basic_recipe["ts"]
