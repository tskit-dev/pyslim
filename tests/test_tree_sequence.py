...
class TestSlimTreeSequence(tests.PyslimTestCase):
    ...
    def test_deprecation(self):
        tables = self.clean_example()
        ts = tables.tree_sequence()
        with pytest.warns(FutureWarning, match="SlimTreeSequence class is deprecated"):
            ts = pyslim.SlimTreeSequence(ts)
...
class TestIndividualMetadata(tests.PyslimTestCase):
    ...
    def test_individual_embellishments(self, recipe):
        ...
        times, populations, ages = pyslim.individual_times_populations_ages(ts)
        assert np.all(
        ...
        assert np.all(
                populations == ts.individual_populations
        )
        for j, ind in enumerate(ts.individuals()):
            assert times[j] == ind.time
            if is_wf:
                assert ages[j] == 0
            else:
                assert ages[j] == ind.metadata["age"]
            assert populations[j] == ind.population
...
class TestIndividualAges(tests.PyslimTestCase):
    ...
    def test_population(self, recipe):
        ...
        for p in range(ts.num_populations):
            ...
            assert set(sub_inds) == set(all_inds[populations == p])
            sub_inds = pyslim.individuals_alive_at(ts, 0, population=[p])
            assert set(sub_inds) == set(all_inds[populations == p])
...
class TestHasIndividualParents(tests.PyslimTestCase):
    ...
    def verify_has_parents(self, ts):
        ...
        times, _, ages = pyslim.individual_times_populations_ages(ts)
        for t in ts.trees():
...
class TestReferenceSequence(tests.PyslimTestCase):
    ...
    TODO: run some of these tests on fewer examples
    '''
    ...
    def test_mutation_at(self, recipe):
        ...
        for _ in range(100):
            ...
            with pytest.warns(FutureWarning):
                assert a == ts.mutation_at(node, pos)
        ...
        if ts.num_mutations > 0:
            ...
            if has_nucleotides:
                ...
                for _ in range(100):
                    ...
                    with pytest.warns(FutureWarning):
                        assert a == ts.nucleotide_at(node, pos)
                    if parent == tskit.NULL:
